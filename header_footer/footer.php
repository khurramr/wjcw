    <footer id="Footer" class="text-center lh-2 fs-11 font-medium pb-3">
        <div class="bg-dark-d text-white py-5">
            <div class="container">
                <div class="row text-md-left text-lg-left d-flex justify-content-center">
                    <div class="col-md-3 col-sm-6">
                        <h5 class="text-success text-uppercase mb-md-4 ml-md-4 pl-md-3">Quick Links</h5>
                                <ul class="mr-5">
                                    <li class="text-success">
                                        <a href="../index.php"aria-current="page">Home</a>
                                    </li>
                                    <li class="">
                                        <a href="../pages/how_it_works.php">How It Works</a>
                                    </li>
                                    <li class="">
                                        <a href="../my_account/join-us.php">Join Us</a>
                                    </li>
                                    <li class=""><a href="../my_account/contact-us.php">Contact Us</a></li>
                                    <li class="">
                                        <a href="../my_account/login.php">Login</a>
                                    </li>
                                </ul>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <h5 class="text-success text-uppercase mb-md-4 ml-md-4 pl-md-3 ">Help Center</h5>
                        <ul id="" class="mr-5">
                            <li><a href="../pages/faq.php">FAQs</a>
                            </li>
                            <li><a href="../pages/terms&conditions.php">Terms &#038; Conditions</a>
                            </li>
                            <li ><a href="../pages/privacy.php">Privacy</a>
                            </li>
                            <li><a href="../pages/disclaimer.php">Disclaimer</a>
                            </li>
                            <!--<li><a href="../pages/help.php">Help</a>-->
                            </li>
                        </ul>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <h5 class="text-success text-uppercase mb-md-4 ml-md-4 pl-md-3">Company</h5>
                        <ul class="mr-5">
                            <li>LONDON UNITED KINGDOM</li>
                            <li><a href="mailto:wjcwelfare110@gmail.com"class="text-success"><u><span class="">wjcwelfare110@gmail.com</span></u></a>
                            </li>
                            <!--<li>+(44) 79032 4 1174</li>-->
                        </ul>
                    </div>
                    <div class="col-md-3 py-3 col-sm-6 ">
                        <h5 class="text-success text-uppercase mb-md-4 ml-md-4 pl-md-3">Follow Us</h5>
                        <ul class="mr-5">
                            <li class="list-inline-item"><a href="#" class="fab fa-facebook fa-2x"></a></li>
                            <li class="list-inline-item"><a href="#" class="fab fa-twitter fa-2x"></a></li>
                            <li class="list-inline-item"><a href="#" class="fab fa-linkedin fa-2x"></a></li>
                            <li class="list-inline-item"><a href="#" class="fab fa-skype fa-2x"></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </footer>
        <small class="copy text-uppercase ">
            <div class="container text-muted pt-3 pb-3 ">
                <div class="row">
                    <div class="col-md-6 col-sm-6 ">
                        &copy; Copyright 2020-<span id="end-date"></span>. <span class="text-success">Wjcw Crowdfunding</span>. | All rights reserved. 
                    </div>
                    <div class="col-md-6 col-sm-6">

                    </div>
                </div>
            </div>
        </small>
        

        <script>
            $("#end-date").html(new Date().getFullYear())
            </script>